...
client.on("guildMemberAdd", member => {
  member.roles.add('839436732549824526'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "839436732595961905");
    const register = "<@&839436732557950989>"
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '✮'
  const sunucu = '839436732520988682'
  const kanal = '845095090690588692'
  const rol = '839436732557950987'
